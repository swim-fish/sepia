...
Run last, after structure is fixed. Evidence: LAMP/CHI 2025 (L), Reinhart et al. PNAS 2025 (P), Russell et al. ACL 2025 (R), Shaib et al. slop taxonomy (S), fiction/RP community ban lists (F), Desaire et al. 2023 (D), Gude et al. 2026 (G), Muñoz-Ortiz et al. 2024 (M), 朱君輝 et al. CCL 2023 on Chinese (Z), Freeburg 2026 (E). Stable source identities live in the repository research ledger; single-letter aliases in this file are file-local. Prescriptions are Sepia design inferences unless a cited source explicitly tested the intervention. The **replace 74% / delete 18% / insert 8%** distribution (L) describes edits observed in that study; it is not a quota or a target for another text. Choose the smallest changes that repair the identified problem while preserving meaning and voice. Add words when clarity, continuity, or supported specificity requires them; use §4 to check for avoidable padding.
...
| 5 | Lack of specificity | Add or substitute real names, objects, numbers, and actions from the supplied material. If you lack the material, ask the user — filling in more generic description makes it worse. |
...
**Restoring is not padding.** Machine editing of human prose leaves a trace of its own, and it is not the words it changes: relative to their human sources, machine-edited texts show a sharp fall in lexical density (share of content words; d = −3.10) with entropy down and lexical diversity barely up — the reverse of the generation footprint, which raises both (Shan et al. 2026, ledger `SHAN-EDIT-2026`; measured on English, extended to other languages as a Sepia inference). These corpus-level results do not establish a word budget for an individual revision. On refactor, use two editorial checks where they help assess changed prose. The **deletion test** asks whether removing an addition preserves meaning, clarity, grammar, and the author's intended voice; cut it if it adds no value. The **reversion test** compares a replacement with the original; keep the original when it already works as well or better for the requested register. Necessary repair and useful clarification stay. Restore the table's forms when they serve the genre or voice, without requiring compensating deletions elsewhere or a fixed final length. These are Sepia editing heuristics, not detector thresholds. Generation and editing leave different traces: §2–3 and §5 examine the former; this paragraph checks avoidable padding during revision.
