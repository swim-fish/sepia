...
| **refactor** | Minimal in-place revision preserving structure, voice, and intent. Identify defects, then fix the necessary ones, deepest layer first. Prefer the smallest effective edit; observed editor ratios are not quotas. The `Voice fit:` suggestion is not a defect or an automatic fix. Before finishing, use the deletion and reversion checks in `references/style-pass.md` §4 to examine additions and replacements; preserve necessary repair. |
